...
class CNNPolicy(nn.Module):
    def __init__(self, frames, num_actions, action_space=None):
    ...
    def sample(self, x, goal, speed):
        mean, logstd = self.forward(x, goal, speed)
        stds = log_stds.exp()
        ...
        # calculate entropies
        logprob = normals.log_prob(x_t) - torch.log(self.action_scale *(1 - actions.pow(2)) + epsilon)
        entropies = log_probs.sum(dim=1, keepdim=True)
        mean = torch.tanh(means) * self.action_scale + self.action_bias
        return actions, entropies, mean

    def to(self, device):
        self.action_scale = self.action_scale.to(device)
        self.action_bias = self.action_bias.to(device)
        return super(CNNPolicy, self).to(device)
...
class ValueNetwork(nn.Module):
    def __init__(self, frames):
        ...
        self.linear1 = nn.Linear(128*32, 256)
        self.linear2 = nn.Linear(256+2+2, 128)
    ...
    def forward(self, state):
        ...
        v = torch.cat((v1, goal, speed), dim=-1)
...
class QNetwork(nn.Module):
    def __init__(self, frames, num_actions):
        ...
        self.crt_fea_cv2 = nn.Conv1d(in_channels=32, out_channels=32, kernel_size=3, stride=2, padding=1)
        self.linear1 = nn.Linear(128*32, 256)
        self.linear2 = nn.Linear(256+2+2+num_actions, 128)
        self.linear3 = nn.Linear(128, 1)
        ...
        self.linear4 = nn.Linear(128*32, 256)
        self.linear5 = nn.Linear(256+2+2+num_actions, 128)
